...
Version:      1.8.19
# Rocky Linux 9 distro identifier (RPM dist tag): .el9
%{!?dist:%global dist .el9}
Release:      9%{?dist}
...
# Rocky Linux 9 ships OpenSSL 3.x. Require it explicitly so the build does
# not pick up the OpenSSL 1.1.1 ABI used by the el7/tl3 targets.
BuildRequires: openssl-devel >= 3.0.0 readline-devel ncurses-devel
...
# The Rocky Linux 9 target folds the documentation back into the main
# package, so obsolete the separately-split ipmitool-help package.
Obsoletes: ipmitool-help < %{version}-%{release}
Provides:  ipmitool-help = %{version}-%{release}
...
%changelog
* Thu Sep 10 2026 dev<dev@smartx.com> - 1.8.19-9
- Add Rocky Linux 9 (el9) build target
- Build against OpenSSL 3.x instead of the OpenSSL 1.1.1 ABI
- Obsolete the separate ipmitool-help package (docs are in the main package)

* Thu Jun 19 2025 dev<dev@smartx.com> - 1.8.19-8
- Fix lan print
